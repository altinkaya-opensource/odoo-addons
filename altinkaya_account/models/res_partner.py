...
class ResPartner(models.Model):
    ...
    def unreconcile_partners_amls(self):
        """Remove reconciliation for partner's AMLs with currencies."""
        if (
    ...
    def calc_currency_valuation(self, move_date):
        """Calculate currency valuation for foreign partners."""
        query = """
            SELECT partner_id,
                   currency_id,
                   account_id,
            ...
            FROM (
                SELECT
                    ...
                    L.account_id,
                    CASE
                        WHEN (SUM(L.debit) - SUM(L.credit)) > 0
                        THEN ROUND((SUM(L.debit) - SUM(L.credit)), 2)
                        ELSE 0.00
                    END AS TRY_DEBIT,
                    CASE
                        WHEN SUM(L.debit) - SUM(L.credit) < 0
                        THEN -1 * ROUND(
                            (SUM(L.debit) - SUM(L.credit)), 2
                        )
                        ELSE 0.00
                    END AS TRY_CREDIT,
