...
class ChooseDeliveryPackage(models.TransientModel):
    ...
    def _compute_move_lines(self):
        for rec in self:
            rec.move_line_ids_without_package = rec.picking_id.move_line_ids_without_package.filtered(lambda l: not l.result_package_id)
    ...
    def action_put_in_pack(self):
        ...
        if not selected_lines:
            raise UserError("Please select product(s) to pack.")

        for line in selected_lines:
            if line.picking_id != self.picking_id:
                raise UserError("Selected lines must belong to the same picking.")
