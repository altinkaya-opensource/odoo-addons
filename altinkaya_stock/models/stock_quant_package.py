...
class StockQuantPackage(models.Model):
    ...
    def action_print_label(self):
        """
        Print the label of the package.
        """
        self.ensure_one()
        if not self.label:
            raise ValidationError(_("This package does not have a label."))

        printer = self.picking_id.carrier_id.default_printer_id
        if not printer:
            raise Warning(_("Please define a default printer for the carrier."))

        printer.print_document(
            report=None,
            content=base64.b64decode(self.label),
        )
