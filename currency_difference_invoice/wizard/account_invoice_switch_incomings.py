    ...
    def switch_invoices(self):
        ...
        if invoices.mapped("journal_id.code") != ["KFARK"]:
            raise ValidationError(
                _("You can only switch curerncy difference invoices.")
            )

        if einvoice.state == "draft":
            raise ValidationError(
                _("You can not switch draft e-invoices. Please validate them first.")
            )
        ...
        currency_diff_aml = old_invoice.mapped(
            "full_reconcile_ids.reconciled_line_ids"
        ).filtered(lambda a: a.journal_id.code == "KRFRK")
        ...
        aml_to_reconcile = (
            old_invoice.mapped("full_reconcile_ids.reconciled_line_ids")
            - currency_diff_aml
        )
