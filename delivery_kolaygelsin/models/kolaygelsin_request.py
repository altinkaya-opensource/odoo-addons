...
KOLAYGELSIN_API_URL = "https://apiintg.kolaygelsin.com"


class KolaygelsinRequest:
    """Interface between Kolay Gelsin Kargo REST API and Odoo recordset
    Abstract Kolay Gelsin API Operations to connect them with Odoo
    ...
    def _shipping_label(self, reference_number, barcode_type):
        ...
        response = self._process_post_request(url="/api/Cargo/GETBARCODE", vals=vals)
        return response
    ...
    def _cancel_shipment(self, reference=False):
        ...
        response = self._process_post_request(
            url="/api/Cargo/CANCELDELIVERY", vals=vals, post_with_params=True
        )
