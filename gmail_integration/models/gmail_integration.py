...
class GmailIntegration(models.Model):
    ...
    mail_ids = fields.One2many("gmail.integration.mail", "gmail_id", string="Çekilen Mailler")
    ...
    def action_fetch_mail(self):
        for rec in self:
            ...
            if not messages:
                raise UserError("Uygun mail bulunamadı.")
            ...
            for msg in messages:
                ...
                try:
                    date = parsedate_to_datetime(raw_date).replace(tzinfo=None) if raw_date else fields.Datetime.now()
                except Exception:
