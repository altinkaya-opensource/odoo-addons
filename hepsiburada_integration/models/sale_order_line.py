...
class SaleOrderLine(models.Model):
    ...
    def _compute_price_unit(self):
        """Skip pricelist computation for HB lines (use API prices)."""
        hb_lines = self.filtered(lambda l: l.order_id.hb_binding_ids)
        return super(SaleOrderLine, self - hb_lines)._compute_price_unit()

    def explode_set_contents(self):
        """Distribute Hepsiburada pack prices across exploded component lines.

        When a phantom BoM line from a Hepsiburada order is exploded, the
        ...
        """
        hb_set_lines = self.filtered(
            lambda l: (
                l.order_id.hb_binding_ids
                and l.set_product
