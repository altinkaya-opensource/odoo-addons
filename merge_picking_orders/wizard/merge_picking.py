...
class MergePicking(models.TransientModel):
    ...
    merge_picking_ids = fields.Many2many(
        "stock.picking", string="Orders", help="Selected orders"
    )
    ...
    def action_merge(self):
        ...
        if (
            len(
                list(
                    set(
                        x.partner_id if x.partner_id else None
                        for x in self.merge_picking_ids
                    )
                )
            )
            > 1
            ...
            raise AccessError(
                _(
                    "Merging is not allowed on Different partners,"
        ...
        if len(list(set(self.merge_picking_ids.mapped("picking_type_id")))) > 1:
            raise AccessError(
                _(
                    "Merging is not allowed on Different picking type,"
        ...
        #     raise AccessError(
        #         _(
        #             "Merging is not allowed on Different State "
        #             "Pickings, please add orders in same State"
        #         )
        #     )
        if len(self.merge_picking_ids) == 1:
            raise AccessError(
                _("Merging is not allowed on Single picking, please add minimum Two")
            )
