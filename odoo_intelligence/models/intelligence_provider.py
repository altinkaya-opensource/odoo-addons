...
class IntelligenceProvider(models.Model):
    ...
    provider_url = fields.Char(string='Provider URL', required=True, default="https://openrouter.ai/api/v1")
    api_key = fields.Char(string='API Key', required=True, help="openai/gpt-3.5-turbo, openai/gpt-4, deepseek-chat...")
    model_name = fields.Char(string='Model', required=True)
    prompt_ids = fields.One2many(
        'intelligence.prompt', 'provider_id', string='Prompts'
    )
    ...
    system_prompt = fields.Text(string="System Prompt", help="This content will be sent before every prompt.")
    ...
    def action_test_prompts(self):
        ...
        for prompt in self.prompt_ids:
            try:
                messages = [
                    {"role": self.system_role, "content": self.system_prompt or "System prompt empty"},
                    {"role": prompt.system_role or 'user', "content": prompt.prompt_text}
                ]
                response = client.chat.completions.create(
                    model=self.model_name,
                    messages=messages,
