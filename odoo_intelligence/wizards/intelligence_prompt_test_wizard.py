...
class IntelligencePromptTestWizard(models.TransientModel):
    ...
    record_id = fields.Many2oneReference(string='Record', model_field='model_id', required=True)
    user_id = fields.Many2one('res.users', string='User', required=True)
    lang = fields.Selection(selection=lambda self: self.env['res.lang'].get_installed(), string='Language', required=True)
    ...
    def _render_body_html(self):
        ...
        return rendered_html.decode('utf-8') if isinstance(rendered_html, bytes) else rendered_html
            ...
            messages = [
                {
                    ...
                    "content": f"{self.prompt_id.prompt_text}{self.lang}" or f"Translate this HTML content to {self.lang}"
                },
                {
                    "role": "user",
