...
class TestStockIncotermExtension(common.TransactionCase):
    ...
    def test_sale_order_invoice(self):
        ...
        self.assertTrue(
            invoice
            and invoice.incoterm.id == self.incoterm.id
            and invoice.transport_type == "air"
        )

    def test_purchase_order_invoice(self):
        self.purchase_order.signal_workflow("purchase_confirm")
        invoice = self.purchase_order.invoice_ids and self.purchase_order.invoice_ids[0]
        self.assertTrue(
            invoice
            and invoice.incoterm.id == self.incoterm.id
            and invoice.transport_type == "maritime"
        )
    ...
    def test_sale_order_picking_invoice(self):
        ...
        self.assertTrue(
            invoices
            and invoices[0].incoterm.id == self.incoterm.id
            and invoices[0].transport_type == "ground"
            and invoices[0].destination_port == "port1"
        )
