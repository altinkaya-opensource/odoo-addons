...
class StockRule(models.Model):
    ...
    mts2_rule_id = fields.Many2one("stock.rule", string="MTS2 Rule")
    ...
    def _check_mts_mto_rule(self):
        for rule in self:
            if rule.action == "split_procurement2":
                if (
                    not rule.mts_rule_id
                    or not rule.mto_rule_id
                ...
                if (
                    ...
                    msg = _(
                        ...
                        "rule: %s! It should not be the same."
                    ) % (rule.name,)
                    raise ValidationError(msg)
    ...
    def _calculate_qtys_mts_mts_mto(self, product, product_qty, product_uom, values):
        ...
        if ((qty_available / product_qty) * 100) < (
            self.do_not_split_percentage or 0.0
        ):
            qty_available = 0.0
        ...
        if float_compare(qty_available, product_qty, precision_digits=precision) < 0:
            ...
            if ((qty_available2 / product_qty) * 100) < (
                self.do_not_split_percentage or 0.0
            ):
                qty_available2 = 0.0
        ...
        if float_compare(total_qty_available, 0.0, precision_digits=precision) > 0:
            if (
                float_compare(
                    total_qty_available, product_qty, precision_digits=precision
                )
                >= 0
            ):
    ...
    def _run_split_procurement2(
        self, product_id, product_qty, product_uom, location_id, name, origin, values
    ):
        precision = self.env["decimal.precision"].precision_get(
            "Product Unit of Measure"
        )
        mts1_qty, mts2_qty, mto_qty = self._calculate_qtys_mts_mts_mto(
        ...
        if float_is_zero(mto_qty, precision_digits=precision):
            if float_is_zero(mts2_qty, precision_digits=precision):  # 1
                getattr(self.mts_rule_id, f"_run_{self.mts_rule_id.action}")(
                    product_id,
                    product_qty,
                    product_uom,
                    location_id,
                ...
                getattr(self.mts2_rule_id, f"_run_{self.mts2_rule_id.action}")(
                    product_id,
                    product_qty,
                    product_uom,
                    location_id,
                ...
                getattr(self.mts_rule_id, f"_run_{self.mts_rule_id.action}")(
                    product_id, mts1_qty, product_uom, location_id, name, origin, values
                )
            ...
            if float_is_zero(mts1_qty + mts2_qty, precision_digits=precision):  # 6
                getattr(self.mto_rule_id, f"_run_{self.mto_rule_id.action}")(
                    product_id, mto_qty, product_uom, location_id, name, origin, values
                )
                ...
                getattr(self.mts2_rule_id, f"_run_{self.mts2_rule_id.action}")(
                    product_id, mts2_qty, product_uom, location_id, name, origin, values
                )
                getattr(self.mto_rule_id, f"_run_{self.mto_rule_id.action}")(
                    product_id, mto_qty, product_uom, location_id, name, origin, values
                )
